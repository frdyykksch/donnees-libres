...
int main() {
    ...
    vector<vector<string>> lignes_Garcons = select_Lignes(selectLignes(t, 0, "M"), 2, prenom);
    vector<vector<string>> lignes_Filles = select_Lignes(selectLignes(t, 0, "F"), 2, prenom);
    ...
    if (lignes_Garcons.size() == 0) {
        ...
        cout << "L'année la plus forte pour les garçons est " << annees_garcons[idx_max_garcons] 
             << " avec " << naissances_garcons[idx_max_garcons] << " naissances" << endl;
    }
